...
exports.adminMiddleware = (req, res, next) => {
  User.findById({
    ...
    if (err || !user) {
      return res.status(400).json({
        errors: 'ব্যবহারকারীর একাউন্ট খুঁজে পাওয়া যায়নি! 😕',
      });
    }

    if (user.role !== 'admin') {
      return res.status(400).json({
        errors: 'এটি এডমিন অ্যাকাউন্ট নয় । অ্যাক্সেস অস্বীকৃত। 🙄',
      });
    }
  ...
  client
    ...
    .then((response) => {
      ...
      if (email_verified) {
        ...
        return res.status(400).json({
          errors: 'গুগল একাউন্ট দিয়ে লগইন ব্যর্থ হয়েছে। আবার চেষ্টা করুন। 😕',
        });
      }
    });
};
